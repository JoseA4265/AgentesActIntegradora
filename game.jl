# ==============================================================================
# AMONG US - CLON MULTIJUGADOR EN JULIA (Server Autoritativo - Fix macOS)
# ==============================================================================
using Sockets
using Serialization
using GLFW
...
# ==============================================================================
# 3. MOTOR GRÁFICO (OPENGL MANUAL PARA EVITAR ERRORES DE LIBRERÍA)
# ==============================================================================
...
# Implementación Manual de Frustum para reemplazar glFrustum
function manual_glFrustum(l, r, b, t, n, f)
    # Matriz de proyección OpenGL (Column-Major)
    M = Float32[
        (2*n)/(r-l)   0.0           0.0            0.0;
        0.0           (2*n)/(t-b)   0.0            0.0;
...
# Implementación Manual de LookAt
...
function render_scene(state::GameState, my_id::Int, window::GLFW.Window)
    ...
    glBegin(GL_QUADS)
    glNormal3f(0,1,0)
    glVertex3f(17, 0.05, -3); glVertex3f(23, 0.05, -3)
    glVertex3f(23, 0.05, 3); glVertex3f(17, 0.05, 3)
    glEnd()
...
if length(ARGS) < 1
    ...
    if ARGS[1] == "server"
        run_server(parse(Int, ARGS[2]))
    elseif ARGS[1] == "client"
        run_client(ARGS[2], parse(Int, ARGS[3]))
    end
end